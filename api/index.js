...
router.get('/msg-latency', async (ctx) => {
  ...
  const getLatencyStats = async (key) => {
    try {
      ...
      const latencies = data.map(item => {
        const { publish_received_at, acked_at } = JSON.parse(item);
        return acked_at - publish_received_at;
      });

      return {
        avg: Math.floor(latencies.reduce((a, b) => a + b, 0) / latencies.length),
  ...
  try {
      // 获取 QoS1 和 QoS2 的延迟统计
      const [qos1Stats, qos2Stats] = await Promise.all([
        getLatencyStats('msg:latency:1'),
  ...
  router.post('/spu-encryption', async (ctx) => {
    ...
    // 直接从响应中解构需要的数据,避免创建中间变量
    const { topic, ...encryptedData } = resp.data
    // 使用 JSON.stringify 和 Buffer.from 的链式调用,减少中间变量
    ctx.body = Buffer.from(JSON.stringify(encryptedData)).toString('base64')
    ctx.status = 200
  })
